...
Future<LotStepsState> lotSteps(Ref ref, String lotId) async {
  ...
  // All DAO reads in parallel — consistent pattern: all via DAO.
  ...
  final (
    ...
    ref.read(washingLocalRepoProvider).getByLotId(lotId),
  ...
  final washDone     = washRecord     != null;
  ...
  // Non-natural:  Cosecha → Clasificación → Despulpado → Fermentación → Lavado → Secado → Catación (7 steps)
  final rawSteps = processType == 'natural'
      ...
      : [
          ...
          (id: 'washing',        label: 'Lavado',         route: AppRoutes.washing,        done: washDone,    active: false),
